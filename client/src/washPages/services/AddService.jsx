...
export default function AddNewServiceForm() {
  ...
  useEffect(() => {
    ...
    const subService = subs.find(
      (s) => String(s.id) === String(serviceData.subServiceId),
    );
    if (subService) {
  ...
  const handleCreateOrUpdate = async () => {
    ...
    const payload = {
      ...
      // 🔥 REAL BILLING VALUES
      partsCost: partsSubtotalCalc + laborSubtotalCalc,
      partsGst: cgstTotalCalc + sgstTotalCalc,
      estimatedTotal: grandTotalCalc,
    };
    ...
    navigate(`/wservices-details/${createdService.id}`);
  };
  ...
  return (
    <div className="min-h-screen p-6 bg-gray-50 dark:bg-gray-900 md:p-10">
      <div className="max-w-5xl mx-auto">
        {/* Header */}
        <div
          className={`flex items-center justify-between px-6 py-5 mb-6 ${isDarkMode ? "bg-gray-800" : "bg-white"} shadow-sm rounded-2xl`}
        >
          <div>
            <h1
        ...
        {/* Main Card */}
        <div
          className={`p-6 space-y-6 ${isDarkMode ? "bg-gray-800" : "bg-white"} shadow-sm rounded-2xl`}
        >
          {/* Client Selection and Name */}
          <div className="grid gap-4 md:grid-cols-2">
            <div className="space-y-2">
              <label
                className={`flex items-center gap-2 text-sm font-medium ${isDarkMode ? "text-gray-200" : "text-slate-800"}`}
              >
              ...
              >
                ...
                {clientsError && <option>{clientsError}</option>}
              </select>
            </div>
          </div>
          ...
          <div className="grid gap-4 md:grid-cols-2">
            ...
            <div className="relative space-y-2">
              ...
              {showCategoryDropdown && (
                <div
                  className={`absolute z-20 w-full mt-1 overflow-auto ${
                    isDarkMode
                ...
                {subServices
                  .filter((s) =>
                    subServiceQuery
                      ? s.name
                          .toLowerCase()
                          .includes(subServiceQuery.toLowerCase())
                      : true,
                  )
                  .map((s) => (
                    <div
          ...
          <div className="space-y-2">
            ...
            <div
              className={`flex items-center overflow-hidden border rounded-lg ${isDarkMode ? "border-gray-600" : "border-slate-300"}`}
            >
              <input
                type="file"
                multiple
              ...
              <span
                className={`px-3 py-2.5 text-sm ${isDarkMode ? "text-gray-400" : "text-slate-400"}`}
              >
              ...
              <div className="grid grid-cols-2 gap-4 mt-3 sm:grid-cols-3">
                {selectedFiles.map((file, index) => (
                  ...
                  >
                    ...
                    <span
                      className={`mt-2 text-xs text-center ${isDarkMode ? "text-gray-300" : "text-slate-700"}`}
                    >
                      {file.name}
                    </span>
                  </div>
          ...
          {/* Footer */}
          <div
            className={`flex items-center justify-between pt-6 border-t ${isDarkMode ? "border-gray-700" : "border-slate-200"}`}
          >
            <div
              className={`text-base font-semibold ${isDarkMode ? "text-gray-200" : "text-slate-800"}`}
            >
              Estimated Total:
            </div>
