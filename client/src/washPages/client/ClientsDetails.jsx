...
export default function WashingClientDetails() {
  ...
  // ✅ STEP 2: Add WhatsApp Handler Function
  async function handleManualWhatsApp() {
    if (!confirm(`Send 'Vehicle Received' message to ${client.phone}?`)) return;
  ...
  if (loading) {
    return (
      ...
      >
        ...
        <p className={`ml-3 text-lg ${isDark ? "text-gray-300" : ""}`}>
  ...
  if (!client) {
    return (
      <div
        className={`p-10 text-center transition-all duration-300 ${isDark ? "bg-gray-900" : ""}`}
      >
        <h2 className={`text-xl ${isDark ? "text-white" : ""}`}>
          Client not found
        </h2>
      </div>
    );
    ...
    >
      ...
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        <div className="flex flex-col gap-4 md:flex-row md:justify-between md:items-center">
          <div className="flex items-center gap-4">
            <button
              ...
              className={`p-3 rounded-xl shadow hover:scale-105 transition-all duration-300 ${
                isDark
          ...
          {/* ✅ STEP 3: Add WhatsApp Button in Header */}
          {/* ACTION BUTTONS */}
          <div className="flex gap-3 flex-wrap">
            {!isEditMode && (
              <button
                onClick={handleManualWhatsApp}
                // disabled={!client.whatsappOptin}
                className={`flex items-center gap-2 px-5 py-3 rounded-xl text-white shadow-lg transition-all duration-300 hover:scale-105
                                    ${
            ...
            {!isEditMode ? (
              ...
              <>
                <button
                  ...
                  className={`px-5 py-3 rounded-xl transition-all duration-300 hover:scale-105 ${
                    isDark
          ...
          <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
            {isEditMode ? (
              ...
              <>
                ...
                {/* ✅ STEP 4: Show WhatsApp Opt-in Status */}
                <div
                  className={`px-4 py-3 rounded-xl text-sm font-semibold shadow-sm flex items-center justify-center md:col-span-2
                                    ${
                                      client.whatsappOptin
        ...
        <Section title="Vehicle Images" icon={<ImageIcon />} isDark={isDark}>
          {/* MAIN IMAGE */}
          <div className="mb-6">
            <h3 className={`mb-3 font-semibold ${isDark ? "text-white" : ""}`}>
              Main Image
            </h3>
          ...
          {client.additionalImages.length > 0 && (
            <>
              <h3
                className={`mb-3 font-semibold ${isDark ? "text-white" : ""}`}
              >
                Additional Images
              </h3>
...
function Section({ title, icon, isDark, children }) {
  return (
    ...
    >
      <h2
        className={`flex items-center gap-2 mb-6 text-xl font-bold ${isDark ? "text-white" : "text-gray-900"}`}
      >
...
function InfoCard({ icon, label, value, isDark }) {
  return (
    <div
      className={`p-4 rounded-xl flex items-center gap-3 shadow transition-all duration-300 hover:scale-105 ${
    ...
    >
      ...
      <div>
        <p
          className={`text-xs font-medium uppercase ${isDark ? "text-gray-400" : "text-gray-500"}`}
        >
          {label}
        </p>
        <p
...
function EditInput({ icon, label, name, value, onChange, isDark }) {
  return (
    <div>
      <label
        className={`flex items-center block gap-2 mb-2 font-semibold ${isDark ? "text-gray-300" : ""}`}
      >
        <span className="text-blue-500">{icon}</span>
        {label}
      </label>
