// client/src/pages/BikeDashboard.jsx
...
export default function BikeDashboard() {
  ...
  useEffect(() => {
    const loadDashboard = async () => {
      try {
        const [servicesRes, invoiceRes, reminderRes] = await Promise.all([
          api.get("/api/bike-services"),
          api.get("/api/bike-invoices"),
          api.get("/api/bike-reminders"),
        ]);
        ...
        setReminders(
          Array.isArray(reminderRes.data?.data)
            ...
            : Array.isArray(reminderRes.data)
            ? reminderRes.data
            : []
        );
      } catch (err) {
  ...
  const inService = services.filter(
    (s) => s.status === "Pending"
  ).length;
  ...
  // ACTIVE REPAIRS
  // ===============================
  const activeRepairs = services.filter(
    (s) => s.status === "Pending"
  );
    ...
    >
      ...
      <div className="mb-8 animate-fade-in">
        <div className="flex items-center justify-between mb-2">
          <div>
            <h1 className="text-4xl py-5 font-bold bg-gradient-to-r from-blue-600 to-blue-700 bg-clip-text text-transparent">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        <StatCard icon={Wrench} label="Total Repairs" value={totalRepairs} isDark={isDark} />
        <StatCard icon={Clock} label="In Service" value={inService} isDark={isDark} />
        <StatCard icon={CheckCircle} label="Completed" value={completed} isDark={isDark} />
        <StatCard icon={IndianRupee} label="Revenue" value={`₹${revenue}`} isDark={isDark} />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
        ...
        <div className={`lg:col-span-2 ${isDark ? "bg-gray-800" : "bg-white"} rounded-2xl shadow-lg p-6`}>
          <div className="flex items-center justify-between mb-6">
            ...
            <span className={`px-3 py-1 ${isDark ? "bg-blue-900/30 text-blue-400" : "bg-blue-100 text-blue-700"} rounded-full text-sm font-semibold`}>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className={`font-semibold ${isDark ? "text-white" : "text-gray-800"}`}>
                        {repair.client?.ownerName}
                      </p>
                      <p className={`text-sm ${isDark ? "text-gray-400" : "text-gray-600"}`}>
                        {repair.client?.regNumber}
                      </p>
                    </div>
                  </div>
              ...
              todayReminders.map((r) => (
                <div
                  ...
                  className={`p-3 rounded-lg ${
                    isDark
                      ? "bg-gray-700/50 border border-gray-600"
                      : "bg-gradient-to-r from-slate-50 to-blue-50 border border-slate-200"
                  }`}
                >
                  <p className={`font-semibold ${isDark ? "text-white" : "text-gray-800"}`}>
                    {r.bike?.ownerName}
