...
export const getReportsSummary = async (req, res) => {
  try {
    ...
    const [invoices, serviceGroups, topClientsGroup, invoiceStatus] =
      await Promise.all([
    ...
    const completedServices = servicesRaw.filter(
      (s) => s.status === "Completed",
    ).length;
    const pendingServices = servicesRaw.filter(
      (s) => s.status === "Pending",
    ).length;
    const cancelledServices = servicesRaw.filter(
      (s) => s.status === "Cancelled" || s.status === "Void",
    ).length;

    const totalServiceRevenue = servicesRaw.reduce(
    ...
    for (const s of servicesRaw) {
      ...
      categoryRevenueMap[key] =
        (categoryRevenueMap[key] || 0) + (Number(s.cost) || 0);
    }
    ...
    res
      .status(500)
      .json({ message: error.message || "Error fetching reports summary" });
  }
};
