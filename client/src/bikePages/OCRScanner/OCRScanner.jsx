...
import OCRUploader from "./components/BikeOCRUploader.jsx";
import OCRResults from "./components/BikeOCRResults.jsx";
import OCRHistory from "./components/BikeOCRHistory.jsx";
import OCRDebugPanel from "./components/BikeOCRDebugPanel.jsx";
import { processImage } from "./utils/BikeOCRProcessor.js";
import {
  saveRecord,
  loadHistory,
  deleteRecord,
  clearHistory,
} from "./utils/BikestorageUtils.js";
import {
  FiFileText,
  FiCheckCircle,
  FiRotateCcw,
...
const OCRScanner = () => {
  ...
  const [bikeClientId, setBikeClientId] = useState(urlBikeClientId || "");
  ...
  useEffect(() => {
    const fetchClients = async () => {
      ...
      try {
        ...
        console.log("Fetching clients from:", `${API_BASE}/api/bikes`);
  ...
  const handleSave = async (data) => {
    try {
     if (!bikeClientId) {
        toast.error("Please select a bike before saving OCR data.");
        return;
      }
      ...
      <div className="max-w-7xl mx-auto space-y-6 sm:space-y-8 ">
                ...
                {loadingClients ? (
                 ...
                 <select
                    ...
                    onChange={(e) => {
                      ...
                      setBikeClientId(selected?.id || "");
                     setClientName(selected?.ownerName || "");
                    }}
                    ...
                    {clientList.length > 0 ? (
                        clientList.map((c) => (
                          <option key={c.id} value={c.id} className="bg-gray-800 text-white">
                            {c.ownerName} ({c.bikeBrand} {c.bikeModel})
                          </option>
                        ))
                      ) : (
        ...
        <div className={`rounded-2xl p-6 shadow-xl transition-all duration-300 hover:shadow-2xl ${isDark ? 'bg-gray-800' : 'bg-white'}`}>
          ...
          <OCRHistory
            ...
            onClear={async () => {
              await clearHistory(bikeClientId);
              setHistoryData([]);
