...
export default function WServiceDetails() {
    ...
    /* ---------------- UI STATES ---------------- */
    if (loading) {
        return <p className={`p-6 text-center ${isDark ? "text-gray-300" : ""}`}>Loading...</p>;
    }

    if (error) {
        return (
            <div className={`p-6 text-center ${isDark ? "text-red-400" : "text-red-500"}`}>
                {error}
                <div className="mt-4">
                    <button
                        onClick={() => navigate(-1)}
    ...
    function Card({ title, children }) {
        return (
            <div className={`p-6 rounded-xl transition-all duration-300 ${isDark
                ...
                {title && (
                    <h2 className={`mb-4 text-xl font-semibold ${isDark ? "text-white" : ""}`}>{title}</h2>
                )}
                {children}
            </div>
        );
    ...
    /* ---------------- MAIN VIEW ---------------- */
    return (
        <div className={`max-w-6xl p-6 mx-auto space-y-6 transition-all duration-300 ${isDark ? "bg-gray-900" : ""}`}>

            {/* Back */}
            <div
                className={`flex items-center cursor-pointer transition-colors ${isDark ? "text-blue-400 hover:text-blue-300" : "text-green-600 hover:text-green-700"
                    }`}
                onClick={() => navigate(-1)}
            >
            ...
            <div className="flex items-center justify-between">
                <div>
                    <h1 className={`flex items-center gap-2 text-3xl font-semibold ${isDark ? "text-white" : ""}`}>
                ...
                <div className="flex gap-3">
                    <button className={`flex items-center px-4 py-2 rounded-lg transition-all duration-300 hover:shadow-md hover:-translate-y-0.5 ${isDark
                    ...
                    <button
                        ...
                        className={`flex items-center px-4 py-2 text-white rounded-lg transition-all duration-300 hover:shadow-md hover:-translate-y-0.5 ${isDark
            ...
            {/* Status */}
            <div
                className={`py-2 text-center text-white rounded-lg font-medium transition-all duration-300 ${service.status === "COMPLETED"
                    ? isDark
                        ? "bg-green-700"
                        : "bg-green-600"
                    : isDark
            ...
            <div className="grid gap-6 md:grid-cols-2">
                <Card title="Service Details">
                    ...
                    <Detail label="Service Sub-Category" value={service.subService?.name} />
            ...
            <Card title="Client Information">
                <div className="grid gap-6 md:grid-cols-2">
                    ...
                    <div className="space-y-2">
                        <p className={`flex items-center gap-2 font-semibold ${isDark ? "text-white" : ""}`}>
            ...
            <Card>
                <div className="flex items-center justify-between">
                    ...
                    <button
                        ...
                        className={`px-6 py-3 text-white rounded-lg transition-all duration-300 hover:shadow-lg hover:-translate-y-0.5 ${isDark
...
function Detail({ label, value }) {
    return (
        <div className="mb-3">
            <p className={`text-sm ${isDark ? "text-gray-400" : "text-gray-500"}`}>{label}</p>
            <p className={`font-medium ${isDark ? "text-gray-200" : ""}`}>{value || "—"}</p>
        </div>
    );
}